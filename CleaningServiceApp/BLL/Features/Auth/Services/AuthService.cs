...
namespace Cleaning.BLL.Features.Auth
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<bool> RegisterAsync(RegisterRequestDto request)
        {
            ...
            string fullNameForEmail = request.FullName;
            string emailForEmail = request.Email;
            string otpCodeForEmail;
            ...
            try
            {
                ...
                var newAccount = new Account
                {
                    ...
                    PasswordSalt = AuthConstants.EmbeddedSaltPlaceholder,
                    Role = parsedRole,
                ...
                _context.VerificationCodes.Add(new VerificationCode
                {
                    ...
                    ExpiresAt = DateTime.UtcNow.AddMinutes(AuthConstants.EmailVerificationOtpExpiryMinutes),
                    IsUsed = false,
                    CreatedAt = DateTime.UtcNow
                });
            ...
            catch (Exception ex)
            {
                ...
                throw new AppException(AppErrors.AuthRegisterFailed, ex);
            }

            string emailBody = $@"
                <h2>Xác thực tài khoản CleanAI</h2>
                <p>Chào {fullNameForEmail},</p>
                <p>Mã OTP xác thực tài khoản của bạn là: <strong style='font-size: 24px;'>{otpCodeForEmail}</strong></p>
                <p>Mã này sẽ hết hạn trong 15 phút.</p>";

            try
            {
                await _emailService.SendEmailAsync(emailForEmail, "Mã xác thực tài khoản - CleanAI", emailBody);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Đăng ký thành công nhưng gửi email xác thực thất bại cho {Email}", emailForEmail);
            }
        ...
        public async Task<AuthResponseDto?> LoginAsync(LoginRequestDto request, string? ipAddress, string? userAgent)
        {
            ...
            {
                throw new AppException(AppErrors.AuthAccountNotActive);
            }
            ...
            _context.RefreshTokens.Add(new RefreshToken
            {
                ...
                TokenHash = tokenHash,
                ExpiresAt = DateTime.UtcNow.AddDays(Convert.ToDouble(_configuration[JwtConfigKeys.RefreshTokenExpirationDays])),
            ...
            return new AuthResponseDto
            {
                ...
                FullName = account.Profile?.FullName ?? AuthConstants.UnknownFullNameFallback
            };
        }
        ...
        public async Task<bool> ForgotPasswordAsync(ForgotPasswordRequestDto request)
        {
            ...
            _context.VerificationCodes.Add(new VerificationCode
            {
                ...
                ExpiresAt = DateTime.UtcNow.AddMinutes(AuthConstants.PasswordResetOtpExpiryMinutes),
                IsUsed = false,
                CreatedAt = DateTime.UtcNow
            });
        ...
        public async Task<bool> ResetPasswordAsync(ResetPasswordDto request)
        {
            ...
            account.PasswordSalt = AuthConstants.EmbeddedSaltPlaceholder;
            account.UpdatedAt = DateTime.UtcNow;
        ...
        public async Task<bool> SendPhoneVerificationOtpAsync(Guid userId)
        {
            ...
            _context.VerificationCodes.Add(new VerificationCode
            {
                ...
                ExpiresAt = DateTime.UtcNow.AddMinutes(AuthConstants.PhoneVerificationOtpExpiryMinutes),
                IsUsed = false,
                CreatedAt = DateTime.UtcNow
            });
        ...
        private string GenerateJwtToken(Account account)
        {
            ...
            var key = Encoding.UTF8.GetBytes(_configuration[JwtConfigKeys.Secret]!);
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                Expires = DateTime.UtcNow.AddMinutes(Convert.ToDouble(_configuration[JwtConfigKeys.AccessTokenExpirationMinutes])),
                Issuer = _configuration[JwtConfigKeys.Issuer],
                Audience = _configuration[JwtConfigKeys.Audience],
        ...
        {
            return RandomNumberGenerator.GetInt32(AuthConstants.OtpMinValue, AuthConstants.OtpMaxValueExclusive).ToString();
        }
        ...
        public async Task<string?> ReauthenticateAsync(Guid userId, string password)
        {
            ...
            {
                new Claim(JwtRegisteredClaimNames.Sub, account.Id.ToString()),
                new Claim(ReauthClaims.TypeClaim, ReauthClaims.ReauthValue)
            };

            var tokenDescriptor = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(claims),
                Expires = DateTime.UtcNow.AddMinutes(AuthConstants.ReauthTokenExpiryMinutes),
                Issuer = _configuration[JwtConfigKeys.Issuer],
                Audience = _configuration[JwtConfigKeys.Audience],
                SigningCredentials = new SigningCredentials(new SymmetricSecurityKey(key), SecurityAlgorithms.HmacSha256Signature)
            };
