...
{
    internal sealed class AiToolExecutor(AppDbContext context, ILogger logger)
    {
                ...
                Function = new GroqFunction
                {
                    ...
                    Parameters = new
                    {
                        ...
                        properties = new
                        {
                            ...
                            {
                                type = "integer",
                                description = $"Số đơn tối đa muốn lấy ({MyBookingsToolLimits.Min}-{MyBookingsToolLimits.Max}), mặc định {MyBookingsToolLimits.Default}"
                            }
                        },
                        required = Array.Empty<string>()
                    }
        ...
        public async Task<string> ExecuteAsync(Guid userId, string toolName, string argumentsJson)
        {
            try
            {
                ...
                return toolName switch
                {
                    ...
                    _ => AiConstants.InvalidToolError
                };
            }
            catch (JsonException ex)
            {
                logger.LogWarning(ex, "Không thể phân tích tham số công cụ AI cho {ToolName}", toolName);
                return AiConstants.InvalidToolError;
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Thực thi công cụ AI {ToolName} thất bại", toolName);
                return AiConstants.QueryFailedError;
            }
        }
        ...
        private async Task<string> GetServicesAsync()
        {
            ...
            return JsonSerializer.Serialize(result, AiJsonOptions.Default);
        }
        ...
        private async Task<string> GetMyBookingsAsync(Guid userId, JsonElement args)
        {
            ...
            var limit = MyBookingsToolLimits.Default;
            if (args.TryGetProperty("limit", out var limitProp) && limitProp.ValueKind == JsonValueKind.Number && limitProp.TryGetInt32(out var requestedLimit))
                limit = Math.Clamp(requestedLimit, MyBookingsToolLimits.Min, MyBookingsToolLimits.Max);
        ...
        private async Task<string> GetServiceDetailAsync(JsonElement args)
        {
            ...
            {
                logger.LogWarning(ex, "Không thể phân tích BookingFormSchema cho ServiceId {ServiceId}", service.Id);
            }
            ...
            return JsonSerializer.Serialize(detail, AiJsonOptions.Default);
        }
        ...
        private async Task<string> GetServiceRatingsAsync(JsonElement args)
        {
            ...
            return JsonSerializer.Serialize(grouped, AiJsonOptions.Default);
        }

        private static string FormatVietnamTime(DateTime utc) =>
            utc.AddHours(VietnamTimeConstants.UtcOffsetHours).ToString("HH:mm 'ngày' dd/MM/yyyy") + " (giờ Việt Nam)";
    }
}
