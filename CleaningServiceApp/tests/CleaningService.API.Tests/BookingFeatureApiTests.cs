...
public sealed class BookingFeatureApiTests(PostgreSqlApiFixture fixture) : IAsyncLifetime
{
    ...
    private async Task SeedAsync()
    {
        ...
        {
            Id = ServiceId, Name = "Schema Service", PropertyType = PropertyType.Apartment,
            UnitType = ServiceUnitType.Hour, BasePrice = 100_000, MinimumHours = 2,
            IsActive = true, BookingFormSchema = Schema, CreatedAt = now, UpdatedAt = now
        });
        ...
        db.WorkerProfiles.Add(new WorkerProfile
        {
            ...
            LocationUpdatedAt = now, CreatedAt = now, UpdatedAt = now
        });
        db.WorkerServices.Add(new Cleaning.DAL.Entities.WorkerService
        {
            WorkerId = WorkerId, ServiceId = ServiceId, IsVerified = true, CreatedAt = now, UpdatedAt = now
        });
