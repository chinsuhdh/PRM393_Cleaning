    ...
    IWorkerPushSender pushSender) : IDispatchPublisher
{
    public async Task JobPostedAsync(BookingDto booking, IReadOnlyCollection<Guid> workerIds)
    {
        ...
        {
            await hub.Clients.Group(DispatchGroups.Worker(workerId)).SendAsync("jobPosted", booking);
    ...
    {
        await hub.Clients.Group(DispatchGroups.Booking(bookingId)).SendAsync("bookingStatusChanged", bookingId, newStatus);
        await hub.Clients.Group(DispatchGroups.Client(clientId)).SendAsync("bookingStatusChanged", bookingId, newStatus);
    }

    public Task WorkerPositionAsync(Guid bookingId, decimal latitude, decimal longitude) =>
        hub.Clients.Group(DispatchGroups.Booking(bookingId)).SendAsync("workerPosition", new
        {
            latitude,
            longitude,
    ...
    public Task NearbyWorkerLocationsAsync(Guid bookingId, IReadOnlyList<NearbyWorkerLocationDto> locations) =>
        hub.Clients.Group(DispatchGroups.Booking(bookingId)).SendAsync("nearbyWorkersUpdated", locations);

    public Task WorkerSuspendedAsync(Guid workerId) =>
        hub.Clients.Group(DispatchGroups.Worker(workerId)).SendAsync("workerSuspended", workerId);

    public Task ChatMessageReceivedAsync(Guid bookingId, BookingMessageDto message) =>
        hub.Clients.Group(DispatchGroups.Booking(bookingId)).SendAsync("receiveMessage", message);

    private async Task SendToWorkersAsync(string eventName, Guid bookingId, IEnumerable<Guid> workerIds)
    {
        foreach (var workerId in workerIds)
            await hub.Clients.Group(DispatchGroups.Worker(workerId)).SendAsync(eventName, bookingId);
    }
}
