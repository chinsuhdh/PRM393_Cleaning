...
namespace CleaningService.API.Features.Profiles
{
    ...
    public class ProfilesController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadAvatar(IFormFile file)
        {
            ...
            {
                throw new AppException(AppErrors.AvatarUploadFailed, ex);
            }
        }
        ...
        public async Task<IActionResult> CompleteOnboarding()
        {
            ...
            return Ok(ApiResponse.Message(ResponseMessages.OnboardingCompleted));
        }
        ...
        public async Task<IActionResult> DeleteAccount([FromHeader(Name = "X-Reauth-Token")] string reauthToken)
        {
            ...
            try
            {
                tokenHandler.ValidateToken(reauthToken, new TokenValidationParameters
                {
                    ...
                    ValidateIssuer = true,
                    ValidIssuer = _configuration[JwtConfigKeys.Issuer],
                    ValidateAudience = true,
                    ValidAudience = _configuration[JwtConfigKeys.Audience],
            ...
            var tokenType = jwtToken.Claims.FirstOrDefault(x => x.Type == ReauthClaims.TypeClaim)?.Value;
            var tokenUserId = jwtToken.Claims.FirstOrDefault(x => x.Type == JwtRegisteredClaimNames.Sub)?.Value;

            if (tokenType != ReauthClaims.ReauthValue || tokenUserId != userId.ToString())
            {
                throw new AppException(AppErrors.Unauthorized);
            }
            ...
            return Ok(ApiResponse.Message(ResponseMessages.AccountDeleted));
        }
    }
}